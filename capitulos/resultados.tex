...
Em termos de comparação relativa ao manual, o DistilBERT (Figura~\ref{fig:tweets-graph-dbert}) tende a ficar mais próximo, sendo o que menos retira a classe ``não rotulado'' (C0: 43{,}3\% e C1: 43{,}2\%). BERT, sBERT e ``que'' (Figuras~\ref{fig:tweets-graph-bert}, \ref{fig:tweets-graph-sbert} e \ref{fig:tweets-graph-que}) tornam as classes rotuladas mais assertivas, principalmente \textit{com desinformação} no C0 e \textit{sem desinformação} no C1. Entre os resultados, o DistilBERT apresenta o desvio mais contido, enquanto BERT/sBERT/``que'' oferecem maior assertividade com menor conservadorismo.
...
O DistilBERT, ilustrado na Figura~\ref{fig:retuites-graph-dbert}, atua como um freio nessa tendência. Ele preserva uma parcela maior de ``não rotulado'' em ambos os clusters, aproximando a distribuição dos retuítes do que se vê no manual. Em C0, isso se traduz em um crescimento mais moderado da classe ``com desinformação''. Em C1, significa manter, em nível relativamente alto, a incerteza capturada pela classe ``não rotulado'', e, portanto, menor risco de super-rotulagem. Em resumo, entre os modelos testados, DistilBERT apresenta o desvio mais contido em relação ao padrão manual, enquanto BERT, sBERT e ``que'' (Figuras~\ref{fig:retuites-graph-bert}, \ref{fig:retuites-graph-sbert} e \ref{fig:retuites-graph-que}) fornecem leituras mais assertivas.
