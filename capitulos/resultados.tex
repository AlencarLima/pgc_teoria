...
Mais especificamente, os valores médios de tempo de processamento por tarefa para sumarização automática foram os seguintes para cada modelo testado:
\begin{itemize}
...
O fator determinante para a escolha do SBERT foi, principalmente, o tempo de processamento reduzido, pois os resultados dos resumos gerados não apresentaram diferenças significativas em relação aos outros modelos testados. Além disso, o SBERT demonstrou desempenho consistente tanto na extração de textos factuais quanto em processos futuros do estudo, como a análise de conteúdo e a comparação de dados.

Combinando a sumarização e a extração de textos factuais, foi possível obter uma visualização mais clara e organizada dos dados, reduzindo ruídos e destacando elementos fundamentais para a análise do conteúdo desinformativo, especialmente no contexto das eleições brasileiras de 2022.
...
\begin{table}[ht]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        Factual   & 0,67 & 0,97 & 0,79 \\
        Não-Factual & 0,99 & 0,86 & 0,92 \\
        \midrule
...
O modelo SVM apresenta uma acurácia global moderada, em torno de 0,79. Para textos não factuais, obteve \textit{precision} de 0,85 e \textit{recall} de 0,83, enquanto para textos factuais, associada às sentenças factuais/relevantes, alcançou \textit{precision} de 0,69 e \textit{f1-score} de 0,71, conforme \autoref{tab:resultados_svm}. Indicando que o modelo é capaz de capturar uma parcela significativa das sentenças de interesse, embora ainda com ocorrência de falsos positivos e perda de algumas sentenças relevantes. Isso significa que a filtragem contribui para reduzir o volume de texto a ser analisado. Contudo, a presença de textos menos relevantes entre os resultados sugere que o modelo SVM pode beneficiar-se de ajustes adicionais ou da combinação com outras técnicas de filtragem para melhorar a qualidade do conteúdo extraído.
...
\begin{table}[ht]
    ...
    \begin{tabular}{lccc}
        ...
        Não-Factual  & 0,8538    & 0,8263 & 0,8398 \\
        \midrule
...
O DistilBERT atua como um freio nessa tendência. O modelo preserva uma parcela maior de ``não rotulado'' em ambos os \textit{clusters}, aproximando a distribuição dos retuítes do que se vê no manual. Em C0, isso se traduz em um crescimento mais moderado da classe ``com desinformação''. Em C1, significa manter, em nível relativamente alto, a incerteza capturada pela classe ``não rotulado'', e, portanto, menor risco de rotulagem excessiva de \textit{tweets} não relevantes. Em resumo, entre os modelos testados, DistilBERT apresenta desvio mais contido em relação ao padrão manual, enquanto BERT, sBERT e ``que'' fornecem leituras mais assertivas.
...
manual e controlar falsos positivos nos retuítes, DistilBERT é mais similar e retorna resultados mais conservadores. Para maximizar a rotulagem e obter uma visão mais clara dos polos de desinformação e informação correta, BERT, sBERT e a segmentação por ``que'' são mais eficazes já que tendem a retornar menos textos não rotulados.
...
O objetivo geral deste trabalho foi investigar em que medida modelos de PLN, em particular arquiteturas baseadas em \textit{transformers}, podem ser empregados para apoiar a detecção de fatos relevantes e potenciais casos de desinformação em textos em língua portuguesa, a partir de dados previamente verificados por agências de \textit{fact-checking}.

Inicialmente, a revisão dos modelos de PLN aplicáveis à detecção de fatos relevantes permitiu mapear as principais abordagens da literatura recente e selecionar um conjunto representativo de modelos para experimentação, incluindo variantes baseadas em \textit{BERT}, tendo o modelo DistilBERT com resultados mais próximos aos gerados pelo método manual. Em seguida, esses modelos foram aplicados a um conjunto de textos verificados, ajustados para o português e contextualizados para a análise de desinformação, incorporando procedimentos de sumarização, segmentação e pré-processamento específicos.

Os resultados empíricos podem ser sintetizados em alguns achados centrais, que ajudam a qualificar o uso de modelos de PLN para detecção de fatos relevantes em português no contexto de desinformação.

Em primeiro lugar, comparação entre diferentes estratégias de segmentação discursiva indicou que abordagens baseadas em conjunções (como a segmentação em torno do termo ``que'') geram segmentos mais alinhados com a noção de ``afirmação checável'' do que alternativas mais restritivas, apresentando 91\% de textos factuais, enquanto os modelos baseados em \textit{transformers} tiveram resultados menos promissores, reduzindo homogeneamente a proporção de factualidade quando comparado ao texto original, girando em torno de 24\% de factualidade. Em contraste, a segmentação baseada apenas em trechos entre aspas apresentou baixa eficácia, produzindo menos segmentos informativos e métricas de classificação inferiores, o que sugere que menções explicitamente citadas não são, por si só, um bom indicador de relevância para checagem no conjunto analisado.

Em segundo lugar, os métodos automáticos realizados apresentaram a tendência de aumentar a proporção de textos classificados como ``com desinformação'' em relação ao método manual de referência. Isso indica que os modelos são mais agressivos na identificação de \textit{tweets}. Entre os modelos testados, o DistilBERT apresentou o desvio mais contido em relação ao padrão manual, enquanto BERT, sBERT e a segmentação por ``que'' forneceram leituras mais assertivas. A escolha entre esses modelos deve considerar o equilíbrio desejado entre assertividade e fidelidade ao padrão manual.
